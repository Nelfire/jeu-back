...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public UserDetailsService userDetailsService(DataSource ds) {
        ...
        manager.setUsersByUsernameQuery("select email, mot_de_passe, 'true' from joueur where email=?");
        manager.setAuthoritiesByUsernameQuery("select c.email, rc.role from joueur c, role_joueur rc where c.id=rc.joueur_id and c.email=?");
        return manager;
    }
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.and().authorizeRequests().antMatchers("/auth").permitAll().antMatchers("/batiment/**").permitAll()
