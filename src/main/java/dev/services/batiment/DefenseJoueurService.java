...
public class DefenseJoueurService {
	 ...
	 * @param defenseRepo
	 */
	public DefenseJoueurService(DefenseJoueurRepo defenseJoueurRepo, JoueurService joueurService, JoueurRepo joueurRepo,
	...
	public List<DefenseJoueur> listerMesDefenses() {
		...
		// PARCOURS DE TOUTES LES DEFENSE DU JOUEUR
	...
	public DefenseJoueurCreationDto construireDefense(DefenseJoueurCreationDto defenseJoueurCreationDto) {
		...
		Integer coutNourritureOperation = defense.getCoutNourritureConstruction()
				* defenseJoueurCreationDto.getQuantite();
		...
		for (DefenseJoueur defJoueur : defenseJoueurRepo.findByJoueur(jou)) {
			// - S'il possède déjà un type de défense, récupération des données (L'id de la
			// défense, et la quantité de défense)
		...
		// SI LE JOUEUR POSSÈDE DÉJÀ CE TYPE DE DEFENSE, ALORS ADDITION LES QUANTITÉS
		// (NOUVELLE + ANCIENNE) + ÉCRASE LA DEFENSE DÉJÀ EXISTANTE
		if (flag == true) {
		...
		if (tempsFinConstruction < new Date().getTime()) {
			...
			finProduction = new Date().getTime()
					+ ((defense.getTempsConstruction() * defenseJoueurCreationDto.getQuantite()) * 1000);
		...
		{
			finProduction = tempsFinConstruction
					+ ((defense.getTempsConstruction() * defenseJoueurCreationDto.getQuantite()) * 1000);
			defenseDuJoueur.setDateDebutConstruction(tempsDebutConstruction);
		}
	...
	public DefenseJoueurDto rechercheDefenseJoueurParId(Integer id) {
		...
		// RECHERCHE D'UNE DEFENSE JOUEUR PAR ID
		DefenseJoueur defenseJoueur = defenseJoueurRepo.findById(id)
				.orElseThrow(() -> new MessageResponseException("La défense joueur n'a pas pu être récupérée"));
		DefenseJoueurDto def = new DefenseJoueurDto(id, defenseJoueur.getJoueur(), defenseJoueur.getDefense(),
				defenseJoueur.getQuantite(), defenseJoueur.getDateDebutConstruction(),
				defenseJoueur.getDateFinConstruction());
	...
	public DefenseJoueurDto accelerationConstructionDefense(@Valid DefenseJoueurDto def, Integer id) {
		...
		DefenseJoueur defenseJoueur = new DefenseJoueur(defenseJoueurDto.getJoueur(), defenseJoueurDto.getDefense(),
				defenseJoueurDto.getQuantite(), defenseJoueurDto.getDateDebutConstruction(),
